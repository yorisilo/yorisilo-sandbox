...
//console.log("canChange : " + canChange.toString(2));	// canChange : 10000
...
Player.prototype.update = function() {
	// console.log("mask : " + this.mask.toString(2));	// 10001か10000
  //console.log("up : " + up.toString(2));	// 1
    //console.log("mask & up :" + (this.mask & up).toString(2)); // 1か0
  if(this.mask & up){
  	
    if (this.mask & canChange) {
    	console.log("mask : " + this.mask.toString(2));	// 10001
  		console.log("canChange : " + canChange.toString(2));	// 10000
    	//console.log("mask & canChange :" + (this.mask & canChange).toString(2)); // 10000
    	//console.log("mask & canChange :" + (this.mask & canChange).toString(10)); // 16
    	
      this.changeForm('previous');
      this.mask &= ~canChange;
      console.log("canChange後のmask : " + (this.mask).toString(2));	// 1
    }
  }
  if(this.mask & down){
    if (this.mask & canChange) {
...
function main() {
  ...
  function handleEvent(event) {
    if(event.type === gamejs.event.KEY_DOWN){ 
    	//console.log("mask : " + player2.mask);	// 16
      //console.log("mask : " + player2.mask.toString(2));	// 10000
      if(event.key === gamejs.event.K_UP){
      	// maskのupをtrueに設定
        player2.mask |= up;
        //console.log("up : " + player2.mask);	// 17
        console.log("up : " + player2.mask.toString(2));	// 10001
      	...
      	// maskのdownをtrueに設定
        player2.mask |= down;
        //console.log("down : " + player2.mask);	// 18
        //console.log("down : " + player2.mask.toString(2));	// 10010
  ...
  gamejs.time.fpsCallback(gameTick, this, 1);
};
